...
class DC_SC_EncoderBlock(nn.Module):
    # DC+SC
    def __init__(
...
class DC_SC_DecoderBlock(nn.Module):
    # DC+SC
    def __init__(
        self,
        pre_channels,
    ...
    def forward(self, inp):
        ...
        for i in range(self.num_layers): # 3
            ...
            input_features = torch.cat([curr_input] + all_outputs[:-1], 1) # 合并
            module = self._modules["conv%d" % i]
            out = module(input_features)
            all_outputs.append(out)
...
class DC_SC_Generator(nn.Module):
    # DC+SC
    ...
    def forward(self, x, z):
        ...
        for i in range(self.U_depth + 1):
            ...
            if i < self.noise_encoders:
                ...
                # print("bbb") # 看噪声拼接后的curr_input

            # print("ccc") # 看pre_input, curr_input
            pre_input, curr_input = self._modules["decode%d" % i](
                [pre_input, curr_input]
            )
            # print("zzzz") # 看decode更新了的pre_input, curr_input
...
# # # # # # # # # # # # # # # #   DC版本  # # # # # # # # # # # # # # # # # # #
class DC_EncoderBlock(nn.Module):
    # DC版
    def __init__(
        self, pre_channels, in_channels, out_channels, num_layers, dropout_rate=0.0
    ):
    ...
    def forward(self, inp):
        ...
        for i in range(1, self.num_layers + 1):
            ...
            all_outputs.append(out)
        return all_outputs[-2], all_outputs[-1]
class DC_DecoderBlock(nn.Module):
    # MC版
    def __init__(
        self,
        pre_channels,
...
################################### SC 版本######################################
class SC_EncoderBlock(nn.Module):
    # SC
    def __init__(
        self, in_channels, out_channels, num_layers, dropout_rate=0.0
    ):
    ...
    def forward(self, inp):
        ...
        for i in range(1, self.num_layers + 1):
            ...
            all_outputs.append(out)
        return all_outputs[-1]
class SC_DecoderBlock(nn.Module):
    # SC
    def __init__(
        self,
        pre_channels,
...
if __name__ == '__main__':
    # 测试是否可以运行
    model = DCSC_Generator(dim=128, channels=1, dropout_rate=0.5)
